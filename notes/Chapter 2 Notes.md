...
## Session History
- 2026-08-15 - Chapter 2 - read in while-still-reading mode (notes/questions dropped as they went), full STEAL batch done after finishing, comprehension + STEAL/technique-targeted questions run at the end
...
### Claude's remarks
- Dewey Decimal System: real thing in real life (library cataloging system), but Jem is just wrong to apply it to Miss Caroline's teaching method, he's a kid confidently misusing adult vocabulary he half-remembers. Good example of Jem's characterization too (overconfident, likes sounding authoritative) if it comes up again later.
- Meridian is a real town in Mississippi, Dill's actual home, only in Maycomb for the summer.
- STEAL + technique should always be stated together, never just the category alone, per user's read on what the test is going to hit hardest given the book-vs-film summative angle.
- Output format going forward: plain chat text by default (no artifacts/SVG unless asked), plot recap chunked scene-by-scene, kept as a fully separate section from the STEAL batch, never merged into one narrative.
- Comprehension checks now always include both plot-recall questions AND at least 3 STEAL/technique-targeted questions, not plot alone, since that's the actual test format.
